...
$(document).ready(function(){
    ...
    const mudaTela = ( atual, nova = null, animacao = "fade", tempoAnimacao = 900 ) => {
        ...
        if(nova == 5){
            var audio = new Audio('imagens2/loveisaverb.MP3');
            audio.volume = 0.1;
            audio.play();
        }
...
function mudaTela(avancar) {
    const telaAtual = document.querySelector('.visivel');
    let proximaTela;
    
    if (avancar) {
        proximaTela = telaAtual.nextElementSibling;
    } else {
        proximaTela = telaAtual.previousElementSibling;
    }

    if (proximaTela) {
        telaAtual.classList.remove('visivel');
        telaAtual.classList.add('invisivel');
        proximaTela.classList.remove('invisivel');
        proximaTela.classList.add('visivel');
    }
}

// Função para exibir as mensagens na nova tela
function exibirMensagens() {
    // Exibir a nova tela
    const telaAtual = document.querySelector('.visivel');
    const telaMensagem = document.getElementById('tela11');
    telaAtual.classList.remove('visivel');
    telaAtual.classList.add('invisivel');
    telaMensagem.classList.remove('invisivel');
    telaMensagem.classList.add('visivel');

    // Mensagens com tempo de exibição
    setTimeout(() => {
        document.getElementById('mensagem1').textContent = "Mensagem 1: Estou tão feliz que você chegou até aqui!";
        document.getElementById('mensagem1').classList.add('show');
    }, 1000);

    setTimeout(() => {
        document.getElementById('mensagem2').textContent = "Mensagem 2: Cada momento com você é muito especial!";
        document.getElementById('mensagem2').style.display = 'block';
        document.getElementById('mensagem2').classList.add('show');
    }, 4000);

    setTimeout(() => {
        document.getElementById('mensagem3').textContent = "Mensagem 3: Obrigado por tudo, você é incrível!";
        document.getElementById('mensagem3').style.display = 'block';
        document.getElementById('mensagem3').classList.add('show');
    }, 7000);
}

// Finalizar e mostrar uma mensagem de agradecimento
function finalizar() {
    alert("Obrigado por passar por todas as etapas!");
    window.location.href = 'http://exemplo.com';  // Redireciona para outra página (opcional)
}

// Adicionando o evento de clique no botão de continuar da tela 10
document.querySelector('.mudaTela').addEventListener('click', exibirMensagens);
